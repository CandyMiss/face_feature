...
namespace YoloV5
{
    ...
    cv::Point get_point(cv::Mat &img, cv::Point& point)
    {
        ...
        {
            x = point.x;
            y = point.y - (Yolo::INPUT_H - r_w * img.rows) / 2;
            x = x / r_w;
            y = y / r_w;
        ...
        {
            x = point.x - (Yolo::INPUT_W - r_h * img.cols) / 2;
            y = point.y;
            x = x / r_h;
            y = y / r_h;
        }
...
// [type] [size] <data x size in hex>
    ...
    ILayer *
    convBnLeaky(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, ITensor &input, int outch,
    ...
    ILayer *
    bottleneckCSP(INetworkDefinition *network, std::map<std::string, Weights> &weightMap, ITensor &input, int c1,
                  int c2,
